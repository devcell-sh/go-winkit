...
func TestBuildInstallCommand_AttachesScratchVolume(t *testing.T) {
	spec := testSpec()
	argv := BuildInstallCommand(spec, "/tmp/windows.iso", "/tmp/autounattend.iso", "/tmp/scratch.img", "")
	joined := strings.Join(argv, " ")

	assert.Contains(t, joined, "file=/tmp/scratch.img,format=raw,if=none,id=scratch0")
	assert.Contains(t, joined, "usb-storage,drive=scratch0,removable=true,bus="+USBBusID+".0")
}
...
func TestBuildInstallCommand_AttachesAutounattendISO(t *testing.T) {
	...
	assert.Contains(t, joined, "file=/tmp/autounattend.iso,media=cdrom,if=none,id=cdrom1")
}

func TestBuildInstallCommand_AttachesDevcellWim(t *testing.T) {
	spec := testSpec()
	argv := BuildInstallCommand(spec, "/tmp/windows.iso", "/tmp/autounattend.iso", "", "/tmp/devcell.qcow2")
	joined := strings.Join(argv, " ")
...
func TestBuildInstallCommand_DevcellWimRaw(t *testing.T) {
	spec := testSpec()
	argv := BuildInstallCommand(spec, "/tmp/windows.iso", "/tmp/autounattend.iso", "", "/tmp/devcell.img")
	joined := strings.Join(argv, " ")
...
func TestBuildInstallCommand_SCSI(t *testing.T) {
	...
	assert.Contains(t, joined, "scsi-cd,drive=cdrom2,bus="+CDBusID+".0")
}
...
func TestBuildInstallCommand_NoAutounattend(t *testing.T) {
	...
	assert.NotContains(t, joined, "scratch0")
}
