...
import (
	...
	"github.com/devcell-sh/go-winkit/vm/vz"
)

var _ vm.VMBackend = (*vz.Backend)(nil)

func TestStub_DefaultDiskFormat(t *testing.T) {
	b := &vz.Backend{}
	if got := b.DefaultDiskFormat(); got != vm.DiskFormatRaw {
...
func TestStub_AllMethodsReturnError(t *testing.T) {
	b := &vz.Backend{}
	if err := b.CreateDisk("/tmp/test.raw", 1, vm.DiskFormatRaw); err == nil {
	...
	if _, err := b.StartInstall(nil, vm.VMInstallConfig{}); err == nil {
		t.Error("StartInstall should error on non-darwin")
	}
	if _, err := b.StartRun(nil, vm.VMRunConfig{}); err == nil {
