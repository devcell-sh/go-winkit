...
func (*Backend) DefaultDiskFormat() vm.DiskFormat { return machine.DiskFormatRaw }

func (*Backend) CreateDisk(path string, sizeGB int, format vm.DiskFormat) error {
	if format == vm.DiskFormatDefault {
		format = vm.DiskFormatRaw
	}
	if format != vm.DiskFormatRaw {
...
func (b *Backend) StartInstall(ctx context.Context, cfg vm.VMInstallConfig) (vm.VM, error) {
	...
	if cfg.DiskPath == "" {
		...
		if err := b.CreateDisk(cfg.DiskPath, sz, vm.DiskFormatRaw); err != nil {
	...
	machine := virtualization.NewVirtualMachineWithConfigurationQueue(vmCfg, queue)
	...
	logger.Info("vz: VM started", "state", vzStateName(machine))

	if vncPort > 0 {
		if err := h.startVNC(machine); err != nil {
...
// vmHandle wraps a virtualization.VZVirtualMachine to satisfy vm.VM.
...
func vzStateName(vm virtualization.VZVirtualMachine) string {
	...
	case virtualization.VZVirtualMachineStateStopping:
		return "stopping"
	default:
		return fmt.Sprintf("unknown(%d)", machine.State())
	}
}
...
func (h *vmHandle) startOnQueue(ctx context.Context) error {
	result, ctxErr := dispatchCall(h.queue, ctx, func(done func(error)) {
		...
		h.machine.StartWithCompletionHandler(done)
	})
	if ctxErr != nil {
		return ctxErr
	}
...
func (h *vmHandle) startVNC(vm virtualization.VZVirtualMachine) error {
	...
	result, displayErr := srv.StartVirtualMachine(machine)
	h.vncServer = srv
...
func (h *vmHandle) stopOnQueue() error {
	result, _ := dispatchCall(h.queue, context.Background(), func(done func(error)) {
		if h.machine.CanRequestStop() {
			if _, err := h.machine.RequestStopWithError(); err == nil {
				select {
				case <-h.done:
					done(nil)
					return
		...
		if h.machine.CanStop() {
			h.machine.StopWithCompletionHandler(done)
			return
		}
		done(nil)
	})
...
func VNCPortFromVM(v vm.VM) uint16 {
...
func ScreenshotFromVM(v vm.VM) ([]byte, error) {
	if h, ok := v.(*vmHandle); ok {
		return h.Screenshot()
	}
