...
import (
	...
	"github.com/devcell-sh/go-winkit/wslnix"
)
...
func wslAnswerConfig(pwshFiles map[string][]byte, opensshName string, opensshData []byte, gosshdName string, gosshdData []byte) unattend.Config {
	...
	// nix.wsl. Without it wsl --import --version 1 exits -1 (run
...
func resolveWSLBackend() (winpe.VMBackend, string, error) {
	...
	registry := map[string]winpe.VMBackend{
		"qemu": &qemu.Backend{},
	}
	if b, ok := vzRegistryBackend(); ok {
		registry["vz"] = b
	}
	return winpe.ResolveBackend(name, registry)
}
...
func buildWSLImage(ctx context.Context, dest, cacheDir, winISO, virtioISO, workDir string, ui *runUI, noCache bool, accel, nixHome string) error {
	...
	if base := os.Getenv("WINKIT_E2E_DISK"); base != "" {
		return continueWSLImage(ctx, base, dest, virtioISO, workDir, ui, accel, backendName, backend)
	}
	...
	// WSL1 Nix rootfs (nix.wsl): built via docker (wslnix), shipped on the
	// answer volume, imported by the first-logon bootstrap. WSL1
	...
	// the preloaded distro (the bootstrap step logs "no nix.wsl" and skips).
	var nixWSLData []byte
	if _, derr := exec.LookPath("docker"); derr == nil {
		nixWsl, nerr := wslnix.BuildTarball(ctx, cacheDir, unattend.SessionUsername(), unattend.DefaultConfig().DistroName, nixHome, noCache,
			func(f string, a ...any) { logger.Info(fmt.Sprintf(f, a...)) })
		if nerr != nil {
			return fmt.Errorf("building nix.wsl: %w", nerr)
		} else if nixWSLData, nerr = os.ReadFile(nixWsl); nerr != nil {
			return fmt.Errorf("reading nix.wsl: %w", nerr)
		}
	} else {
		logger.Warn("docker not on PATH; skipping the preloaded WSL1 Nix distro")
	}
	...
	cfg.NixWSLPayloadData = nixWSLData
	...
	if err := backend.CreateDisk(dest, wslDiskSizeGB, winpe.DiskFormatDefault); err != nil {
	...
	qmpSock := qemu.QMPSocketFromVM(vm)
	...
	if err := waitForWindowsSSH(ctx, provAddr, provUser, provPass, wslInstallWait, logger, vm.Done()); err != nil {
		close(stopTail)
	...
	if len(nixWSLData) > 0 {
		if err := wslVerifyDistro(ctx, cfg.Username, cfg.Password, cfg.DistroName, logger); err != nil {
...
// wslVerifyDistro confirms the bootstrap-imported WSL1 distro runs nix,
...
func wslVerifyDistro(ctx context.Context, user, pass, distro string, logger interface{ Info(string, ...any) }) error {
	addr := fmt.Sprintf("127.0.0.1:%d", wslOpenSSHHostPort)
	deadline := time.Now().Add(10 * time.Minute)
	var lastOut, lastErr string
	for {
		out, errb, code, err := sshRun(ctx, addr, user, pass,
			// -lc: login shell sources /etc/profile -> profile.d/nix.sh,
			// putting nix on PATH; init's default PATH has no nix.
			fmt.Sprintf(`$env:WSL_UTF8='1'; wsl.exe -d %s -e /bin/sh -lc "nix --version"`, distro))
		if err == nil && code == 0 && strings.Contains(decodeWSLOutput(out), "nix (Nix)") {
			logger.Info(distro+" verified", "nix", strings.TrimSpace(decodeWSLOutput(out)))
			break
		}
...
func continueWSLImage(ctx context.Context, base, dest, virtioISO, workDir string, ui *runUI, accel, backendName string, backend winpe.VMBackend) error {
	logger := ui.Logger
	if accel == "" {
		accel = os.Getenv("WINKIT_E2E_ACCEL")
	}
	...
	if err := waitForWindowsSSH(ctx, provAddr, provUser, provPass, 4*time.Minute, logger, vm.Done()); err == nil {
		...
		if err2 := waitForWindowsSSH(ctx, osshAddr, dc.Username, dc.Password, wslContinueWait-4*time.Minute, logger, vm.Done()); err2 == nil {
			provUp = true
	...
	if interactive {
		...
		} else if vncPort := vzVNCPortFromVM(vm); vncPort > 0 {
			logger.Info("  screenshots:           " + filepath.Join(installOut, "screenshots"))
	...
	bakedExt := ".qcow2"
	if backend.DefaultDiskFormat() == winpe.DiskFormatRaw {
		bakedExt = ".raw"
	}
