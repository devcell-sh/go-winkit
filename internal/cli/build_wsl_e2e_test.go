...
import (
	...
	"github.com/devcell-sh/go-winkit/testutil"
...
func TestQcowBuilderWSL(t *testing.T) {
	...
	_, backendName, err := resolveWSLBackend()
	if err != nil {
	...
	if backendName == "qemu" {
		...
		go captureScreenshots(qmpSock, shotDir, stopShots, shotsDone, t)
	...
	if backendName == "qemu" &&
		...
		// Project root is 3 levels up from internal/cli/
...
// captureScreenshots polls the install VM's QMP socket and writes a PNG every
// 30s until stop is closed, so a run can be inspected visually after the fact.
// Failures are logged, never fatal: a missing frame must not fail the install.
func captureScreenshots(qmpSock, shotDir string, stop <-chan struct{}, done chan<- struct{}, t *testing.T) {
	defer close(done)
	seq := 0
	...
	shoot := func() {
		...
		if err := qemu.QMPScreendump(qmpSock, ppm); err != nil {
			t.Logf("screendump %d: %v", seq, err)
			return
		}
		png := ppm[:len(ppm)-3] + "png"
		if err := qemu.ConvertPPMtoPNG(ppm, png); err == nil {
